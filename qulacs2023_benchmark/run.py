...
class BenchmarkCase:
    ...
    def run_benchmark(self) -> BenchmarkResult:
        ...
        for n_qubits in range(self.n_qubits_begin, self.n_qubits_end + 1):
            mount_config = f"type=bind,source=$(pwd)/benchmark/{self.directory},target=/benchmark"
            image_tag = f"qulacs2023_benchmarks/{self.directory}:latest"
            subprocess.run(["docker", "run", "--rm", "-it", "--gpus", "all", "--mount", mount_config, image_tag, "/benchmark/build.sh"], capture_output=True)
            output = subprocess.run(["docker", "run", "--rm", "-it", "--gpus", "all", "--mount", mount_config, image_tag, "/benchmark/main"], capture_output=True)
