...
void update_with_dense_matrix_single_target(Kokkos::View<CTYPE*> state_kokkos, UINT n_qubits, UINT target, Kokkos::View<CTYPE*> matrix_kokkos) {
    const ITYPE lower_mask = (1ULL << target) - 1;
    const ITYPE upper_mask = ~lower_mask;
    Kokkos::parallel_for(1ULL << (n_qubits - 1), KOKKOS_LAMBDA (const ITYPE& it) {
        ITYPE i = (it & upper_mask) << 1 | (it & lower_mask);
        ITYPE j = i | 1ULL << target;
        auto state_updated_i = matrix_kokkos[0] * state_kokkos[i] + matrix_kokkos[1] * state_kokkos[j];
        auto state_updated_j = matrix_kokkos[2] * state_kokkos[i] + matrix_kokkos[3] * state_kokkos[j];
        state_kokkos[i] = state_updated_i;
        state_kokkos[j] = state_updated_j;
    });
}

void update_with_dense_matrix_double_target(Kokkos::View<CTYPE*> state_kokkos, UINT n_qubits, UINT target0, UINT target1, Kokkos::View<CTYPE*> matrix_kokkos) {
    ...
    Kokkos::parallel_for(1ULL << (n_qubits - 2), KOKKOS_LAMBDA (const ITYPE& it) {
        ...
        for(UINT i = 0; i < 4; i++) for(UINT j = 0; j < 4; j++) {
            new_state[i] += matrix_kokkos[i*4+j] * state_kokkos[base[j]];
        }
        for(UINT i = 0; i < 4; i++) {
...
double single_target_matrix_bench(UINT n_qubits) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for(int j = 0; j < 2; j++) {
            Kokkos::deep_copy(Kokkos::subview(matrix, std::make_pair(j*2, (j+1)*2)), Kokkos::subview(matrixes, i, j, Kokkos::ALL));
        }
...
double double_target_matrix_bench(UINT n_qubits) {
    ...
    decltype(targets)::HostMirror targets_host = Kokkos::create_mirror_view(targets);
    auto start_time = std::chrono::high_resolution_clock::now();
    Kokkos::View<CTYPE*> matrix("matrix", 16);
    for (int i = 0; i < 10; ++i) {
        UINT target0 = targets_host(i, 0), target1 = targets_host(i, 1);
        for(int j = 0; j < 4; j++) {
            Kokkos::deep_copy(Kokkos::subview(matrix, std::make_pair(j*4, (j+1)*4)), Kokkos::subview(matrixes, i, j, Kokkos::ALL));
        }
